        ...
        sqlPendingOrder = f"""
            ...
            INNER JOIN (
                SELECT
                    ...
                    COALESCE(SUM(
                        CASE
                            WHEN ord.DiscountPercent > 0 THEN (((ORDLine.Price * ORDLine.RemainingOpenQuantity) + (ORDLine.Price * ORDLine.RemainingOpenQuantity) * ord.DiscountPercent ) / 100)
                            ELSE (ORDLine.Price * ORDLine.RemainingOpenQuantity)
                        END
    ...
    try:
        ...
        if Order.objects.filter(pk = OrderID).exists():
            ...
            for item in allitem:
                ...
                # else:
                #     PendingAmount = float(item.UnitPrice) * float(item.RemainingOpenQuantity)
                # PendingAmount = float(item.UnitPrice) * float(item.RemainingOpenQuantity)
