...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
try:
    settings.custome_error_logs('cron start', module_name='ap_incoming_payments_status')
        
    currentDate = date.today() - timedelta(days=10)
    invCount = requests.get(settings.SAPURL+"/VendorPayments/$count", cookies=ses.cookies, verify=False).text
    ...
    for i in range(count):
        baseUrl = settings.SAPURL+"/VendorPayments?$orderby = DocEntry asc&$select=DocEntry,DocNum,DocType,CardCode,CardName,JournalRemarks&$skip="+str(skip)
        print(baseUrl)
        ...
        for opt in opts['value']:
            ...
            docSelectQuery = f"select * from PurchaseInvoices_vendorpayments WHERE DocEntry = '{DocEntry}'"
            ...
            if mycursor.rowcount > 0:                
                ...
                ord_sql = f"UPDATE `PurchaseInvoices_vendorpayments` SET `JournalRemarks`='{JournalRemarks}' WHERE DocEntry = '{DocEntry}'"
                print(ord_sql)
        ...
        print(skip)
    # endfor
    settings.custome_error_logs('cron end', module_name='ap_incoming_payments_status')
except Exception as e:
    print(str(e))
    settings.custome_error_logs(message=str(e), module_name='ap_incoming_payments_status')