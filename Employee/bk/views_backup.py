...
def login(request):
    try:
        ...
        if Employee.objects.filter(Q(userName=request.data['userName']) | Q(Email=request.data['userName']), Active = "tYES").exists():
            ...
            empId = employee_obj.id
            # if Maps.objects.filter(Emp_Id=empId, shape = 'meeting').exists():
            #     map_obj = Maps.objects.filter(Emp_Id = empId, shape = 'meeting').order_by("-id")[0]
            #     checkInStatus = map_obj.type
