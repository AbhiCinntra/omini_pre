    ...
    try:
        ...
        for lead in leads:
            ...
            if Lead.objects.filter(phoneNumber=phoneNumber).exists():
                ...
                model=Lead(date=date, location=location, companyName=companyName, numOfEmployee=numOfEmployee, turnover=turnover, source=source, status=status, leadType=leadType, contactPerson=contactPerson, designation=designation, phoneNumber=phoneNumber, message=message, email=email, productInterest=productInterest, assignedTo_id=assignedTo_id, employeeId_id=employeeId_id, timestamp=timestamp, projectAmount = projectAmount, customerType = customerType, stages = stages)
                model.save()
