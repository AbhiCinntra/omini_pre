...
def all_filter(request):
    try:
        BusinessPlaceID = request.data['BusinessPlaceID']
        BusinessPlaceIDArr = BusinessPlaceID.split(",")
        Warehouses_obj = Warehouse.objects.filter(BusinessPlaceID__in = BusinessPlaceIDArr, Inactive = "tNO").values("BusinessPlaceID","WarehouseCode","WarehouseName")
        allwr = WarehouseSerializer(Warehouses_obj, many=True)
        return Response({"message": "Success","status": 200,"data":allwr.data})
    except Exception as e:
        return Response({"message":str(e),"status":201,"data":[]})
