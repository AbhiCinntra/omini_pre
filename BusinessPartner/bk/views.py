...
def bp_receipt(request):
    try:
        ...
        # allPaymentsList = []
        # if str(FromDate) != "":
        #     # allPaymentsList = IncomingPayments.objects.filter(InvoiceDocEntry__in = docEntrys, DocDate__gte = FromDate, DocDate__lte = ToDate).values_list('TransferSum', flat=True)
        ...
        # else:
        #     # allPaymentsList = IncomingPayments.objects.filter(InvoiceDocEntry__in = docEntrys).values_list('TransferSum', flat=True)
