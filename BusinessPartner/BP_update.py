...
serBPCount = requests.get(settings.SAPURL+"/BusinessPartners/$count", cookies=ses.cookies, verify=False).text
...
if True:
    ...
    for i in range(count):
        ...
        # bpRes = requests.get(data['sapurl']+'/BusinessPartners/$count?$filter = GroupCode eq 112&$skip='+str(skip), cookies=r.cookies, verify=False)
        # bpRes = requests.get(settings.SAPURL+"/BusinessPartners?$filter = CardType eq 'cCustomer' Valid eq 'tYES'&$skip="+str(skip), cookies=ses.cookies, verify=False)
        ...
        for bp in bps['value']:
            ...
            bpcode = bp['CardCode']
            # print(f'bpcode : {bpcode} not exist in local db')
            CreatedBy = 1
            FreeDelivery = 1
            # Unit = "PatancheruBR"
