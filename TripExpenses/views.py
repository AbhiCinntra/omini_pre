...
from Expense.views import showExpense
...
def all_tripexpenses(request):
    try:
        ...
        # result = showTripExpenses(tripObj)
...
def one_tripexpenses(request):
    try:
        TripId = request.data['id']
        if TripExpenses.objects.filter(pk = TripId).exists():
            tripObj = TripExpenses.objects.filter(pk = TripId)
...
# >>> showTripExpenses >>>>
# >>>>>>>>>>>>>>>>>>>>>>>>>
def showTripExpenses(objs):
    allTripExpenses = []
    for obj in objs:
        tripExpJson = TripExpensesSerializer(obj, many=False)
        finalTripExpense = json.loads(json.dumps(tripExpJson.data))
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        if Expense.objects.filter(tripId = obj.id).exists():
            expObj = Expense.objects.filter(tripId = obj.id)    
            # expJson = ExpenseSerializer(expObj, many=True)
            expData = showExpense(expObj)
            finalTripExpense['Expenses'] = expData
        else:
            finalTripExpense['Expenses'] = []
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        allTripExpenses.append(finalTripExpense)
