...
def create(request):
    ...
    try:
        ...
        LineNum = 0
        for line in lines:
            model_lines = DocumentLines(LineNum = LineNum, QuotationID = qt.id, Quantity = line['Quantity'], UnitPrice = line['UnitPrice'], DiscountPercent = line['DiscountPercent'], ItemCode = line['ItemCode'], ItemDescription = line['ItemDescription'], TaxCode = line['TaxCode'], FreeText = line['FreeText'], UomNo = line['UomNo'], UnitWeight = line['UnitWeight'])
            model_lines.save()
            LineNum=LineNum+1
    ...
    try:
        ...
        for line in lines:
            ...
            if line["id"] !="":
                ...
                model_lines = DocumentLines(QuotationID = fetchid, LineNum=NewLine, Quantity = line['Quantity'], UnitPrice = line['UnitPrice'], DiscountPercent = line['DiscountPercent'], ItemCode = line['ItemCode'], ItemDescription = line['ItemDescription'], TaxCode = line['TaxCode'], FreeText = line['FreeText'], UomNo = line['UomNo'], UnitWeight = line['UnitWeight'])
                model_lines.save()
        ...
        qt_data = {
            ...
            "Comments": request.data['Comments'],
            "SalesPersonCode": request.data['SalesPersonCode'],
            "BPL_IDAssignedToInvoice": request.data['Unit'], #default only one branch exist in SAP
            "PaymentGroupCode":request.data['PayTermsGrpCode'],
            # "U_PORTAL_NO":qt.id,
            "AddressExtension": {
                "BillToBuilding": request.data['AddressExtension']['BillToBuilding'],
...
def showQuote(objs):
    ...
    for obj in objs:
        ...
        if contactType != "":
            contactTypeObj = BPEmployee.objects.filter(InternalCode = contactType).values("id","FirstName","E_Mail")
