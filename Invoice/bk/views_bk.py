...
def showInvoice(objs):
    ...
    for obj in objs:
        ...
        if DocumentStatus == "bost_Close":
            PaymentStatus = "Paid"
        elif IncomingPaymentInvoices.objects.filter(InvoiceDocEntry = invId).exists():
...
def bp_credit_note(request):
    # try:
        ...
        if BusinessPartner.objects.filter(CardCode = CardCode).exists():
            ...
            if str(FromDate) != "":
                creditNotesList = CreditNotes.objects.filter(CancelStatus="csNo", CardCode = CardCode, DocDate__gte = FromDate, DocDate__lte = ToDate).values("id","DocTotal", "CreateDate", "VatSum", "DocEntry", "DocDate", "DiscountPercent", "Comments").order_by('-DocDate')
            else:
                creditNotesList = CreditNotes.objects.filter(CancelStatus="csNo", CardCode = CardCode).values("id","DocTotal", "CreateDate", "VatSum", "DocEntry", "DocDate", "DiscountPercent", "Comments").order_by('-DocDate')
...
def pending_payment_collection(request):
    try:
        ...
        inv_obj = []
        if str(FromDate) != "":
            inv_obj = Invoice.objects.filter(CancelStatus="csNo", DocumentStatus = 'bost_Open', DocDate__gte = FromDate, DocDate__lte = ToDate)
        else:
            inv_obj = Invoice.objects.filter(CancelStatus="csNo", DocumentStatus = 'bost_Open')
