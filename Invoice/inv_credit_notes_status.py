...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
try:
    settings.custome_error_logs('cron start', module_name='inv_credit_notes_status')
    ...
    invCount = requests.get(settings.SAPURL+"/CreditNotes/$count?$filter=UpdateDate ge "+str(currentDate), cookies=ses.cookies, verify=False).text
    ...
    if int(count) > 0:
        ...
        for i in range(count):
            ...
            baseUrl = settings.SAPURL+"/CreditNotes?$orderby = DocEntry desc&$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus,PaidToDateSys&$filter=UpdateDate ge '"+str(currentDate)+"'&$skip="+str(skip)
            print(baseUrl)
            ...
            skip = skip+20
            print(skip)
    settings.custome_error_logs('cron end', module_name='inv_credit_notes_status')
except Exception as e:
    print(str(e))
    settings.custome_error_logs(message=str(e), module_name='inv_credit_notes_status')