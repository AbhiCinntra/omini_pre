...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
try:
    settings.custome_error_logs('cron start', module_name='inv_status')

    # currentDate = '2023-03-28'
    currentDate = date.today() - timedelta(days=2)
    ...
    if int(count) > 0:
        ...
        for i in range(count):
            baseUrl = settings.SAPURL+"/Invoices?$orderby = DocEntry asc&$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus,PaidToDateSys,DocDate,DocDueDate,TaxDate,CreationDate&$filter=PaidToDateSys gt 0 and UpdateDate ge '"+str(currentDate)+"'&$skip="+str(skip)
            print(baseUrl)
            ...
            for opt in opts['value']:
                ...
                if mycursor.rowcount > 0:                
                    ...
                    ord_sql = f"UPDATE `Invoice_invoice` SET `DocumentStatus`='{DocumentStatus}', `CancelStatus` = '{CancelStatus}', `PaidToDateSys` = '{PaidToDateSys}',`DocDate` = '{DocDate}',`DocDueDate` = '{DocDueDate}',`TaxDate` = '{TaxDate}',`CreationDate` = '{CreationDate}' WHERE DocEntry = '{DocEntry}'"
                    print(ord_sql)
            ...
            skip = skip+20
            print(skip)
    settings.custome_error_logs('cron end', module_name='inv_status')
except Exception as e:
    print(str(e))
    settings.custome_error_logs(message=str(e), module_name='inv_status')