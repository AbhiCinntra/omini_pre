...
if True:
    ...
    while skip != "": 
        ...
        sapAPIUrl = f"/CreditNotes?$filter = DocEntry eq {lastDocEntry}"
        ...
        for opt in opts['value']:
            try:
                ...
                if mycursor.rowcount != 1:
                    ...
                    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    seriesNameURL = f"http://103.190.95.182:8000/Ledure/General/SeriesName.xsjs?DBName=RG_Industries_Live_&DocEntry={DocEntry}&ObjType=14"
                    print(seriesNameURL)
