...
# invCount = requests.get(data["sapurl"]+"/Orders/$count?$filter=DocumentStatus ne 'bost_Open' and UpdateDate ge "+str(currentDate), cookies=r.cookies, verify=False).text
invCount = requests.get(data["sapurl"]+"/Orders/$count", cookies=r.cookies, verify=False).text
...
if int(count) > 0:
    ...
    for i in range(count):
        res = requests.get(data["sapurl"]+"/Orders?$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus&$skip="+str(skip), cookies=r.cookies, verify=False)
        ...
        for opt in opts['value']:
            ...
            if mycursor.rowcount > 0:                
                ...
                ord_sql = f"UPDATE `Order_order` SET `DocumentStatus`='{DocumentStatus}', `CancelStatus` = '{CancelStatus}' WHERE DocEntry = '{DocEntry}'"
                print(ord_sql)
