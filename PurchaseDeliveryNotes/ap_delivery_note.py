...
lastDocEntry = 0
# lastDocEntry = 10189
mycursor.execute("SELECT * FROM `PurchaseDeliveryNotes_purchasedeliverynotes` ORDER BY `id` desc LIMIT 1")
...
if True:
    ...
    while skip != "": 
        # res = requests.get(data['sapurl']+'/PurchaseDeliveryNotes?$filter=UpdateDate ge '+str(currentDate)+'&$skip='+str(skip)+'', cookies=r.cookies, verify=False)
        sapAPIUrl = f"/PurchaseDeliveryNotes?$orderby = DocEntry asc&$filter = DocEntry gt {lastDocEntry}&$skip = {skip}"
        #print(sapAPIUrl)
        ...
        for opt in opts['value']:
            ...
            # PDN_ID = str(opt['U_PORTAL_NO']) # local order id
        
            docSelectQuery = f"select * from PurchaseDeliveryNotes_purchasedeliverynotes WHERE DocEntry = '{DocEntry}'"
            ...
            if mycursor.rowcount != 1:
                ...
                add_sql = f"INSERT INTO `PurchaseDeliveryNotes_addressextension`(`PDN_ID`, `ShipToStreet`, `ShipToBlock`, `ShipToBuilding`, `ShipToCity`, `ShipToZipCode`, `ShipToCounty`, `ShipToState`, `ShipToCountry`, `ShipToAddress2`, `ShipToAddress3`, `BillToStreet`, `BillToBlock`, `BillToBuilding`, `BillToCity`, `BillToZipCode`, `BillToCounty`, `BillToState`, `BillToCountry`, `BillToAddress2`, `BillToAddress3`, `PlaceOfSupply`, `PurchasePlaceOfSupply`, `U_SCOUNTRY`, `U_SSTATE`, `U_SHPTYPB`, `U_BSTATE`, `U_BCOUNTRY`, `U_SHPTYPS`) VALUES ('{PDN_ID}','{ShipToStreet}','{ShipToBlock}','{ShipToBuilding}','{ShipToCity}','{ShipToZipCode}','{ShipToCounty}','{ShipToState}','{ShipToCountry}','{ShipToAddress2}','{ShipToAddress3}','{BillToStreet}','{BillToBlock}','{BillToBuilding}','{BillToCity}','{BillToZipCode}','{BillToCounty}','{BillToState}','{BillToCountry}','{BillToAddress2}','{BillToAddress3}','{PlaceOfSupply}','{PurchasePlaceOfSupply}','{U_SCOUNTRY}','{U_SSTATE}','{U_SHPTYPB}','{U_BSTATE}','{U_BCOUNTRY}','{U_SHPTYPS}');"
                #print(add_sql)                
                ...
                for line in opt['DocumentLines']:
                    ...
                    line_sql = "INSERT INTO `PurchaseDeliveryNotes_documentlines`(`LineNum`, `PDN_ID`, `Quantity`, `UnitPrice`, `DiscountPercent`, `ItemDescription`, `ItemCode`, `TaxCode`, `BaseEntry`, `TaxRate`, `UomNo`, `LineTotal`, `U_UTL_DIST`, `U_UTL_SP`, `U_UTL_DD`, `U_UTL_SD`, `U_UTL_TD`, `U_UTL_MRPI`, `U_RateType`,`MeasureUnit`, `HSNEntry`, `SACEntry`, `HSN`, `SAC`, `WarehouseCode`) VALUES ('"+str(line['LineNum'])+"', '"+str(PDN_ID)+"', '"+str(line['Quantity'])+"', '"+str(line['UnitPrice'])+"', '"+str(lDiscountPercent)+"', '"+str(line['ItemDescription'])+"', '"+str(line['ItemCode'])+"', '"+str(line['TaxCode'])+"', '"+str(BaseEntry)+"', '"+str(TaxRate)+"', '"+str(line['UoMCode'])+"', '"+str(line['LineTotal'])+"','"+str(line['U_UTL_DIST'])+"','"+str(line['U_UTL_SP'])+"','"+str(line['U_UTL_DD'])+"','"+str(line['U_UTL_SD'])+"','"+str(line['U_UTL_TD'])+"','"+str(line['U_UTL_MRPI'])+"','"+str(line['U_RateType'])+"','"+str(line['MeasureUnit'])+"','"+str(HSNEntry)+"','"+str(SACEntry)+"','"+str(HSN)+"','"+str(SAC)+"','"+str(WarehouseCode)+"');"
                ...
                # endfor
                sqlInvUpd = f"UPDATE `PurchaseDeliveryNotes_purchasedeliverynotes` SET `IGST` = '{IGST}', `CGST` = '{CGST}', `SGST` = '{SGST}', `GSTRate` = '{GSTRate}' WHERE `id` = '{PDN_ID}'"
                print(sqlInvUpd)
            ...
            # endif
                ...
                ord_sql = f"UPDATE `PurchaseDeliveryNotes_purchasedeliverynotes` SET `DocumentStatus`='{DocumentStatus}', `CancelStatus` = '{CancelStatus}' WHERE DocEntry = '{DocEntry}'"
                print(ord_sql)
                mycursor.execute(ord_sql)
                mydb.commit()
